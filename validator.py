...
colsToConsider=["ColA","ColB"]  #put pk/compositekey to consider
secondaryPk=["ColA-1","ColA-2"]
Stringcol = ["ColC"]
class WriteToExcel():
    workSheet=None
    columnCode=1
    ...
    def printLegendery(self):
        ...
        for aColourKey in colourKeyList:
            ...
            if(aColourKey!="Headers"):
                ...
                self.printItLine()
        self.printItLine()
        self.printItLine("Data Comparison:- 1010 ~ Redshift")
        self.printItLine("Data Format for mismatch cells <1010> != <Redshift>")
    ...
    for row in reader:
        ...
        if hashVal in DataWithHashes:
            ...
            try:
                ...
                hashVal = hashVal +row[secondaryPk[0]]
            except:

                hashVal = hashVal + row[secondaryPk[1]]
...
def columnMapFinder(colsFromJson,mappings,mainfileName, comparefileName): #Auto Column matcher
    bestMappedList = open(mapFile, "wb")
...
def dataCompare(xmlFile,csvFile,file_nm):
    ...
    print "Loading started {}".format(csvFile)
